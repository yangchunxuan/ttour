...
Depends on the sibling contract modules:
    agent.browser.BrowserSession   (session.ensure_login / goto / frames / page / close)
    agent.dom.observe(session)     -> DomState with .to_prompt(), .page_text, .url
    agent.llm.Planner              (planner.decide / planner.extract)
    agent.actions.execute(...)     -> ActionResult
    agent.actions.dismiss_popups(session)
...
The loop adds, beyond the bare ReAct cycle:
  ...
  * a structured transcript, optional on_step callback, and AgentResult
    (rotated cookies persist via BrowserSession.close() -> save_state())
"""
...
from agent.dom import observe as default_observe
from agent.actions import execute as default_execute, dismiss_popups as default_dismiss_popups
from agent.health import assess_page_health as default_health
...
class ReActAgent:
    ...
    def _finish(
        ...
        """Uniform exit. (Rotated cookies are persisted by BrowserSession.close(),
        which the CLI always reaches via its finally block.)"""
        return AgentResult(
