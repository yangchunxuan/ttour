"""agent/llm.py — DeepSeek 规划器（Planner）。

职责：
  - decide(): 把 GOAL + DOM 快照 + 最近历史组织成消息，调用 deepseek-chat
    (response_format=json_object, temperature=0)，再把返回内容
    解析 / 校验 / 修复成 {"thought": str, "action": {"name", "args"}}。
  - extract(): 复用项目里已有的 extract_information_json 做结构化抽取。
...
class Planner:
    ...
    def _build_user_message(self, goal: str, dom_prompt: str, history: list) -> str:
        ...
        parts.append("# CURRENT PAGE（当前页面 DOM 与可见文本）")
